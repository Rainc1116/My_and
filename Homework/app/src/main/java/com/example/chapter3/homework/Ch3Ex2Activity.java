...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator s1 = ObjectAnimator.ofFloat(target,"scaleX",1f,2f);
        ObjectAnimator s2 = ObjectAnimator.ofFloat(target,"scaleY",1f,2f);
        s1.setRepeatCount(ObjectAnimator.INFINITE);
        s1.setRepeatMode(ObjectAnimator.REVERSE);
        s2.setRepeatCount(ObjectAnimator.INFINITE);
        s2.setRepeatMode(ObjectAnimator.REVERSE);
        // TODO ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator a1 = ObjectAnimator.ofFloat(target,"alpha",1f,0.5f);
