...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
         int hourDegree = (int)(30*((hour-3)+(float)minute/60));
        int minDegree = (int)(6*((minute-15)+(float)second/60));
         int secDegree = (6*((second-15)));
         float endXh = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree)));
        float endYh = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree)));
        float endXm = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minDegree)));
        float endYm = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minDegree)));
        float endXs = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secDegree)));
        float endYs = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secDegree)));
    needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
         canvas.drawLines(new float[]{
            centerX,centerY,endXh,endYh,
         },needlePaint);
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLines(new float[]{
                centerX,centerY,endXm,endYm,
        },needlePaint);
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLines(new float[]{
                centerX,centerY,endXs,endYs,
        },needlePaint);
